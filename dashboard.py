...
st.sidebar.markdown("Masukkan API Key Anda untuk mengaktifkan fitur analisis AI.")
api_key = st.sidebar.text_input(
    label="Google AI API Key:",
    type="password",
    help="Dapatkan API Key gratis Anda dari Google AI Studio."
)
st.sidebar.markdown("[🔗 Dapatkan API Key di sini](https://aistudio.google.com/app/apikey)")

# Memberi pemisah visual yang jelas
st.sidebar.markdown("---")

# Filter dikelompokkan di bawah setelah setup AI
with st.sidebar.expander("🔍 Filter Data", expanded=True):
    st.markdown("Atur filter untuk melihat data spesifik.")
    selected_platforms = st.multiselect('Platform:', options=df_original['Platform'].unique(), default=df_original['Platform'].unique())
    all_influencers = df_original['Influencer'].unique()
    selected_influencers = st.multiselect('Influencer:', options=all_influencers, default=all_influencers)
    min_date = df_original['Date'].min().date()
    max_date = df_original['Date'].max().date()
    selected_date_range = st.date_input('Rentang Tanggal:', value=(min_date, max_date), min_value=min_date, max_value=max_date)
...
if not df_filtered.empty:
    ...
    with col1:
        ...
        ax1.set_xlabel('Total Engagements'); ax1.set_ylabel('Platform')
        st.pyplot(fig1)
    with col2:
        ...
        fig2, ax2 = plt.subplots(); ax2.pie(sentiment_counts, labels=sentiment_counts.index, autopct='%1.1f%%', startangle=140, colors=pie_colors); ax2.axis('equal')
        st.pyplot(fig2)
    ...
    fig3, ax3 = plt.subplots(figsize=(10, 6)); sns.barplot(x=influencer_engagement.values, y=influencer_engagement.index, palette=green_palette, orient='h', ax=ax3)
    ax3.set_xlabel('Total Engagements'); ax3.set_ylabel('Influencer')
    st.pyplot(fig3)
else:
    ...
    if not api_key:
        ...
        try:
            ...
            with st.spinner('AI (Gemini) sedang menganalisis data...'):
                response = model.generate_content(prompt); st.markdown(response.text)
